        ...
        if (mbi->mods_count > 0) {
            ...
            if (mbi->mods_count > 0) {
                ...
                printf("  Initrd: 0x%x - 0x%x (%d bytes)\n",
                       initrd_module->mod_start, initrd_module->mod_end,
    ...
    if (mbi->flags & MULTIBOOT_FLAG_VBE) {
        ...
        if (vbe) {
            ...
            printf("  VBE: %dx%dx%d pitch=%d fb=0x%x\n", vbe_fb_width,
                   vbe_fb_height, vbe_fb_bpp, vbe_fb_pitch, vbe_fb_addr);
        }
    }
...
multiboot_module_t *multiboot_get_initrd(void) { return initrd_module; }
