...
int keyboard_buffer_is_enabled(void) { return kb_enabled; }
...
uint8_t keyboard_buffer_pop(void) {
    uint8_t key = 0;
    if (kring_u8_pop(&key_buffer, &key) < 0)
        return 0;
    return key;
}
...
static void keyboard_irq_handler(uintptr_t number __attribute__((unused)),
    ...
    /* PS/2 keyboard data port is 0x60 on all x86 variants. */
    ...
    if (kb_extended) {
        ...
        if (!(scancode & 0x80)) {
            if (scancode == 0x49) {
                arch_terminal_scroll_up();
                return;
            } else if (scancode == 0x51) {
                arch_terminal_scroll_down();
                return;
