...
static kernel_window_t *win_get(int wid) {
    ...
    if (!win->active)
        return NULL;
    if (win->generation != WIN_GEN(wid))
        return NULL;
    return win;
}

void window_init(void) { memset(windows, 0, sizeof(windows)); }
...
int window_getkey(int wid, uint32_t pid) {
    ...
    uint8_t key = 0;
    if (kring_u8_pop(&win->key_ring, &key) < 0) {
        cpu_irq_restore(flags);
        return 0;
    }
    cpu_irq_restore(flags);
...
int window_sendkey(int wid, uint8_t key) {
    ...
    if (kring_u8_push(&win->key_ring, key) < 0) {
        cpu_irq_restore(flags);
        return -1;
    }
    cpu_irq_restore(flags);
...
int window_append_text(int wid, const char *data, int len) {
    if (!data || len <= 0)
        return -1;
    unsigned int flags = cpu_irq_save();
    // Use slot directly — children writing here don't have the
    // generation-encoded wid, they have the raw wid from stdout_wid which IS
    // generation-encoded
    kernel_window_t *win = win_get(wid);
    if (!win) {
        cpu_irq_restore(flags);
    ...
    int written = 0;
    for (int i = 0; i < len; i++) {
        if (kring_u8_push(&win->text_ring, (uint8_t)data[i]) < 0)
            break; // Full, drop excess
        written++;
    }
    cpu_irq_restore(flags);
    return written;
