...
static inline int validate_user_ptr(uint32_t ptr, uint32_t size) {
    ...
    if (ptr < USER_REGION_START)
        return 0; // below user region
    if (size > USER_REGION_END - USER_REGION_START)
        return 0; // impossibly large
    if (ptr + size < ptr)
        return 0; // overflow
    if (ptr + size > USER_REGION_END)
        return 0; // exceeds user region
    return 1;
}

// Validate a null-terminated string pointer. Walks until NUL or bounds
// exceeded. Returns 1 if a NUL terminator is found within the user region, 0
// otherwise.
...
// Yield to scheduler
static void sys_do_yield(void) { task_yield(); }
...
uint32_t load_elf_into(struct page_directory *page_dir, const char *filename,
    ...
    // Avoid leaking kernel heap on large/repeated spawns: kmalloc/kfree
    // currently sits on a bump allocator with no real free. Read ELF into
    // PMM-backed temp frames that we can explicitly release after loading.
    vfs_stat_t st;
    ...
    uint32_t ph_end =
        elf->e_phoff + (uint32_t)elf->e_phnum * sizeof(elf32_phdr_t);
    ...
    for (int i = 0; i < elf->e_phnum; i++) {
        ...
        if (vaddr < USER_REGION_START || vaddr + memsz < vaddr ||
            ...
            printf(
                "[exec] ELF segment %d: vaddr 0x%x+0x%x outside user region\n",
                i, vaddr, memsz);
            pmm_free_frames(temp_frames_base, temp_frames_count);
            return 0;
        }
...
static int sys_do_exec(const char *filename, iret_frame_t *frame) {
    ...
    if (flen >= sizeof(kfilename))
        flen = sizeof(kfilename) - 1;
...
static void sys_do_gfx_exit(void) {
    ...
    task_t *current = task_current();
    if (!current || current->id != gfx_owner_pid)
        return;
...
//   new format: (bpp << 24) | (width << 12) | height   (width/height are
//   12-bit) old userland can still use SYS_GFX_INIT pointer and hardcoded
//   assumptions.
...
static int sys_do_readdir(const char *path, uint32_t index, char *buf,
                          uint32_t size) {
    if (!buf || size == 0)
        return 0;
    task_t *cur = task_current();
...
static uint32_t sys_do_sbrk(int32_t increment) {
    ...
    if (old_brk < current->user_brk_min)
        old_brk = current->user_brk_min;
    uint32_t new_brk = old_brk;

    if (increment > 0) {
        new_brk = old_brk + (uint32_t)increment;
        if (new_brk < old_brk)
            return (uint32_t)-1; // overflow
    } else if (increment < 0) {
        // Keep it simple for now: no shrinking.
        return (uint32_t)-1;
    }

    if (new_brk < current->user_brk_min)
        return (uint32_t)-1;
    if (new_brk >= USER_STACK_BASE_VADDR)
        return (uint32_t)-1;
...
static int sys_do_kill(uint32_t task_id) { return task_kill(task_id, -9); }
...
uint32_t syscall_handler(uint32_t eax, uint32_t ebx, uint32_t ecx, uint32_t edx,
    ...
    case SYS_SPAWN:
        ...
        return (uint32_t)sys_do_spawn((const char *)ebx, (const char **)ecx,
                                      (int)edx);
    ...
    case SYS_READDIR:
        ...
        // Buffer size fixed at 32 (matches FAT16 8.3 names and typical user
        // buffers)
        if (ebx && !validate_user_string(ebx))
            return (uint32_t)-1;
        if (!validate_user_ptr(edx, 32))
            return (uint32_t)-1;
        return (uint32_t)sys_do_readdir((const char *)ebx, ecx, (char *)edx,
                                        32);
    ...
    case SYS_WIN_CREATE: {
        if (ecx && !validate_user_string(ecx))
            return (uint32_t)-1;
        int w = (int)(ebx >> 16);
    ...
    case SYS_WIN_DESTROY: {
        task_t *cur = task_current();
        return cur ? (uint32_t)window_destroy((int)ebx, cur->id) : (uint32_t)-1;
    }
    ...
    case SYS_WIN_GETKEY: {
        task_t *cur = task_current();
        return cur ? (uint32_t)window_getkey((int)ebx, cur->id) : (uint32_t)-1;
    }
    ...
    case SYS_WIN_LIST:
        if (ecx > 0 &&
            !validate_user_ptr(ebx, (uint32_t)ecx * sizeof(win_info_t)))
            return (uint32_t)-1;
    ...
    case SYS_TASKLIST:
        if (ecx > 0 &&
            !validate_user_ptr(ebx, (uint32_t)ecx * sizeof(taskinfo_entry_t)))
            return (uint32_t)-1;
    ...
    case SYS_WIN_READ_TEXT: {
        if (edx > 0 && !validate_user_ptr(ecx, edx))
            return (uint32_t)-1;
        task_t *cur = task_current();
        return cur ? (uint32_t)window_read_text((int)ebx, cur->id, (char *)ecx,
                                                (int)edx)
                   : (uint32_t)-1;
    }
    ...
    case SYS_GETMOUSE: {
        if (ebx && !validate_user_ptr(ebx, 4))
            return (uint32_t)-1;
        if (ecx && !validate_user_ptr(ecx, 4))
            return (uint32_t)-1;
    ...
    case SYS_STAT: {
        if (!validate_user_string(ebx))
            return (uint32_t)-1;
        if (!validate_user_ptr(ecx, sizeof(vfs_stat_t)))
            return (uint32_t)-1;
