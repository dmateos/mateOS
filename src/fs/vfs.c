...
static void vfs_copy_path(char *dst, const char *src) {
    int i = 0;
    for (; i < VFS_PATH_MAX - 1 && src && src[i]; i++)
        dst[i] = src[i];
    dst[i] = '\0';
}
...
int vfs_get_registered_fs_count(void) { return fs_count; }

const char *vfs_get_registered_fs_name(int idx) {
    if (idx < 0 || idx >= fs_count || !filesystems[idx] ||
        !filesystems[idx]->name) {
        return "(null)";
    }
    return filesystems[idx]->name;
}

int vfs_get_virtual_file_count(void) { return virtual_file_count; }
...
static int vfs_path_matches_virtual(const char *path, const char *name) {
    if (!path || !name)
        return 0;
    // Match "mos/foo" or "/mos/foo" against stored name "mos/foo"
...
static int vfs_virtual_index_from_fs_id(int fs_id) {
    ...
    if (index < 0 || index >= virtual_file_count)
        return -1;
    return index;
}
...
int vfs_seek(vfs_fd_table_t *fdt, int fd, int offset, int whence) {
    ...
    int fs = fdt->fds[fd].fs_id;
    if (fs < 0 && fs > VFS_VIRT_BASE_ID)
        return -1; // console fd, not seekable
    int vfi = vfs_virtual_index_from_fs_id(fs);
    if (vfi >= 0) {
        ...
        case SEEK_SET:
            ...
            break;
        case SEEK_CUR:
            pos = fdt->fds[fd].fs_handle + offset;
            break;
        case SEEK_END:
            pos = size + offset;
            break;
...
int vfs_stat(const char *path, vfs_stat_t *st) {
    ...
    // /mos virtual directory
    if (strcmp(path, "/mos") == 0 || strcmp(path, "/mos/") == 0 ||
        strcmp(path, "mos") == 0) {
        st->size = 0;
        st->type = VFS_DIR;
        return 0;
...
// Check if path refers to the /mos virtual directory
...
int vfs_readdir(const char *path, int index, char *buf, uint32_t size) {
    ...
    // Reading /mos: list virtual kernel info files (strip "mos/" prefix)
    if (vfs_is_mos_dir(path)) {
        if (remaining < virtual_file_count) {
            const char *name = virtual_files[remaining].name;
            // Strip "mos/" prefix from stored name for display
            if (name[0] == 'm' && name[1] == 'o' && name[2] == 's' &&
                name[3] == '/')
                name += 4;
...
int vfs_unlink(const char *path) {
    if (!path)
        return -1;
    if (vfs_find_virtual_file(path) >= 0)
        return -1;

    for (int fs = 0; fs < fs_count; fs++) {
        if (!filesystems[fs]->unlink)
            continue;
        if (filesystems[fs]->unlink(path) == 0)
            return 0;
    }
    return -1;
}
...
int vfs_mkdir(const char *path) {
    ...
    for (int fs = 0; fs < fs_count; fs++) {
        if (!filesystems[fs]->mkdir)
            continue;
        if (filesystems[fs]->mkdir(path) == 0)
            return 0;
    }
    return -1;
}
...
int vfs_rmdir(const char *path) {
    ...
    for (int fs = 0; fs < fs_count; fs++) {
        if (!filesystems[fs]->rmdir)
            continue;
        if (filesystems[fs]->rmdir(path) == 0)
            return 0;
    }
    return -1;
}
