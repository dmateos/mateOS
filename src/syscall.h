...
#define SYS_SOCK_RECV                                                          \
    31 // sock_recv(fd, buf, len) -> bytes, 0=closed, -1=no data
#define SYS_SOCK_CLOSE 32    // sock_close(fd) -> 0
#define SYS_WIN_READ_TEXT 33 // win_read_text(wid, buf, max_len) -> bytes read
#define SYS_WIN_SET_STDOUT                                                     \
    34                    // win_set_stdout(wid) -> 0 (-1 to reset to console)
...
typedef struct {
    ...
    uint32_t start_ticks;  // tick count when task was spawned (100Hz)
    char name[32];
} taskinfo_entry_t;
...
static inline int sys_exec(const char *filename) {
    int ret;
    __asm__ volatile("int $0x80" : "=a"(ret) : "a"(SYS_EXEC), "b"(filename));
    return ret;
}

static inline uint32_t sys_gfx_init(void) {
    uint32_t ret;
    __asm__ volatile("int $0x80" : "=a"(ret) : "a"(SYS_GFX_INIT) : "memory");
    return ret;
}
