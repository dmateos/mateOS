...
int strbuf_append_cstr(char *dst, uint32_t cap, uint32_t *len, const char *s) {
  if (!s) return -1;
  while (*s) {
    if (strbuf_append_char(dst, cap, len, *s++) < 0) return -1;
  }
  return 0;
}

int strbuf_append_dec_u32(char *dst, uint32_t cap, uint32_t *len, uint32_t v) {
  if (v == 0) return strbuf_append_char(dst, cap, len, '0');
  ...
  for (int i = t - 1; i >= 0; i--) {
    if (strbuf_append_char(dst, cap, len, tmp[i]) < 0) return -1;
  }
  return 0;
}

int strbuf_append_hex_u32(char *dst, uint32_t cap, uint32_t *len, uint32_t v) {
  if (strbuf_append_cstr(dst, cap, len, "0x") < 0) return -1;
  ...
  for (int shift = 28; shift >= 0; shift -= 4) {
    ...
    if (strbuf_append_char(dst, cap, len, c) < 0) return -1;
  }

  if (!started) {
    if (strbuf_append_char(dst, cap, len, '0') < 0) return -1;
  }
  return 0;
}
