...
static void terminal_redraw(void) {
    ...
    for (size_t y = 0; y < TTY_HEIGHT; y++) {
        // lines_back from cursor_line: how many lines before the current cursor
        // line does this screen row represent? Screen row terminal_row =
        // cursor_line (0 lines back) Screen row 0 = terminal_row lines back
        // from cursor_line With scroll_offset, we shift everything up by
        // scroll_offset more lines
        ...
        if (lines_back < 0 || lines_back >= (int)total_lines) {
            ...
            for (size_t x = 0; x < TTY_WIDTH; x++) {
                terminal_buffer[y * TTY_WIDTH + x] =
                    vga_entry(' ', vga_entry_color(VGA_COLOR_LIGHT_GREY,
                                                   VGA_COLOR_BLACK));
            }
        } else {
