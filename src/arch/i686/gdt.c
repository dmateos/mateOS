...
static void init_gdt_table(gdt_entry_t *gdt) {
  ...
  // Access: Present(1), DPL=0, Type=1(code/data), Executable(1), Direction(0), RW(1), Accessed(0)
  // = 1 00 1 1010 = 0x9A
  ...
  // Access: Present(1), DPL=0, Type=1(code/data), Executable(0), Direction(0), RW(1), Accessed(0)
  // = 1 00 1 0010 = 0x92
  gdt_set_entry(gdt, 2, 0, 0xFFFFFFFF, 0x92, 0xCF);

  // User code segment (index 3, selector 0x18, RPL=3 -> 0x1B)
  // Access: Present(1), DPL=3, Type=1(code/data), Executable(1), Direction(0), RW(1), Accessed(0)
  // = 1 11 1 1010 = 0xFA
  gdt_set_entry(gdt, 3, 0, 0xFFFFFFFF, 0xFA, 0xCF);

  // User data segment (index 4, selector 0x20, RPL=3 -> 0x23)
  // Access: Present(1), DPL=3, Type=1(code/data), Executable(0), Direction(0), RW(1), Accessed(0)
  // = 1 11 1 0010 = 0xF2
