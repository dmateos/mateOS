...
typedef enum { OP_NONE = 0, OP_REG, OP_IMM, OP_LABEL, OP_MEM } op_kind_t;
...
enum { SEC_TEXT = 0, SEC_RODATA = 1, SEC_DATA = 2, SEC_BSS = 3, SEC_COUNT = 4 };
...
static int find_label_addr(asm_ctx_t *ctx, const char *name,
                           unsigned int *addr_out) {
    int idx = get_label_index(ctx, name, 0);
    if (idx < 0 || !ctx->labels[idx].defined)
        return 0;
    *addr_out = ctx->org + ctx->sec_base[ctx->labels[idx].section] +
                ctx->labels[idx].offset;
    return 1;
}
...
static int get_label_index(asm_ctx_t *ctx, const char *name, int create) {
    for (int i = 0; i < ctx->label_count; i++) {
        if (streq(ctx->labels[i].name, name))
            return i;
    }
    if (!create)
        return -1;
...
static int define_label(asm_ctx_t *ctx, const char *name, unsigned int offset,
                        int section) {
    int idx = get_label_index(ctx, name, 1);
    if (idx < 0)
        return 0;
...
static int resolve_label(asm_ctx_t *ctx, const char *name, int *out) {
    int idx = get_label_index(ctx, name, 0);
    if (idx < 0 || !ctx->labels[idx].defined) {
        if (ctx->fmt_obj && ctx->pass == 2 && idx >= 0 &&
            ctx->labels[idx].is_extern) {
            *out = 0;
            return 1;
        }
        if (ctx->pass == 2) {
    ...
    *out = (int)(ctx->org + ctx->sec_base[ctx->labels[idx].section] +
                 ctx->labels[idx].offset);
    return 1;
}
...
static int emit8(asm_ctx_t *ctx, unsigned int v) {
    if (ctx->pass == 2) {
        ...
        if (!ctx->sec_out[s] || ctx->sec_len[s] >= ctx->sec_cap[s]) {
            ...
            if (new_cap < ctx->sec_len[s] + 1)
                new_cap = ctx->sec_len[s] + 1;
            unsigned char *nb = (unsigned char *)realloc(ctx->sec_out[s],
            ...
            if (new_cap > ctx->sec_cap[s]) {
                memset(nb + ctx->sec_cap[s], 0,
                       (unsigned int)(new_cap - ctx->sec_cap[s]));
            }
            ctx->sec_out[s] = nb;
...
static int emit32(asm_ctx_t *ctx, unsigned int v) {
    return emit8(ctx, v & 0xFFu) && emit8(ctx, (v >> 8) & 0xFFu) &&
           emit8(ctx, (v >> 16) & 0xFFu) && emit8(ctx, (v >> 24) & 0xFFu);
}

static int emit_modrm(asm_ctx_t *ctx, int mod, int reg, int rm) {
    return emit8(
        ctx, (unsigned int)(((mod & 3) << 6) | ((reg & 7) << 3) | (rm & 7)));
}
...
static int parse_mem_expr(char *expr, operand_t *op) {
    ...
    while (buf[i]) {
        ...
        if (reg_code_bits(term, &rc, &rb) && rb == 32) {
            ...
            if (sign < 0)
                return 0; // label subtraction not supported in phase-1
            if (op->mem.disp_label[0])
                return 0;
...
static int emit_rm_operand(asm_ctx_t *ctx, int reg_field, operand_t *rmop) {
    ...
    if (rmop->mem.base_reg == -1) {
        if (!emit_modrm(ctx, 0, reg_field, 5))
            return 0;
        int addr = rmop->mem.disp;
        if (rmop->mem.disp_label[0]) {
            int l = 0;
            if (!resolve_label(ctx, rmop->mem.disp_label, &l))
                return 0;
            addr += l;
            unsigned int off = ctx->sec_pc[ctx->cur_sec];
            if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                           MOBJ_RELOC_ABS32, rmop->mem.disp_label,
                           rmop->mem.disp)) {
                return 0;
            }
            if (ctx->fmt_obj) {
                // Relocation addend already carries disp, keep encoded
                // placeholder zero.
                addr = 0;
            }
        }
    ...
    if (!emit_modrm(ctx, mod, reg_field, rm))
        return 0;
    ...
    if (mod == 1)
        return emit8(ctx, (unsigned int)(disp & 0xFF));
    if (mod == 2 || (mod == 0 && base == 5))
        return emit32(ctx, (unsigned int)disp);
    return 1;
}
...
static int is_jcc(const char *m, int *cc) {
    struct {
        ...
        {"jnp", 11}, {"jpo", 11}, {"jl", 12},  {"jnge", 12}, {"jge", 13},
        {"jnl", 13}, {"jle", 14}, {"jng", 14}, {"jg", 15},   {"jnle", 15}};
    for (unsigned int i = 0; i < sizeof(table) / sizeof(table[0]); i++) {
        if (streqi(m, table[i].n)) {
            *cc = table[i].c;
            return 1;
        }
    }
...
static int is_setcc(const char *m, int *cc) {
    struct {
        const char *n;
        int c;
    } table[] = {{"seto", 0},   {"setno", 1},  {"setb", 2},   {"setc", 2},
                 {"setnae", 2}, {"setnb", 3},  {"setnc", 3},  {"setae", 3},
                 {"sete", 4},   {"setz", 4},   {"setne", 5},  {"setnz", 5},
                 {"setbe", 6},  {"setna", 6},  {"seta", 7},   {"setnbe", 7},
                 {"sets", 8},   {"setns", 9},  {"setp", 10},  {"setpe", 10},
...
static int encode_binop_rm_reg(asm_ctx_t *ctx, unsigned char opc, operand_t *a,
                               operand_t *b) {
    // a = r/m, b = reg
    if (!emit8(ctx, opc))
        return 0;
...
static int encode_binop_reg_rm(asm_ctx_t *ctx, unsigned char opc, operand_t *a,
                               operand_t *b) {
    // a = reg, b = r/m
    if (!emit8(ctx, opc))
        return 0;
...
static int encode_grp1_imm(asm_ctx_t *ctx, int ext, operand_t *dst, int imm,
                           int bits) {
    if (bits == 8) {
        if (!emit8(ctx, 0x80))
            return 0;
        if (!emit_rm_operand(ctx, ext, dst))
            return 0;
        return emit8(ctx, (unsigned int)(imm & 0xFF));
    }
    if (imm >= -128 && imm <= 127) {
        if (!maybe_prefix_16(ctx, bits))
            return 0;
        if (!emit8(ctx, 0x83))
            return 0;
        if (!emit_rm_operand(ctx, ext, dst))
    ...
    if (!emit_rm_operand(ctx, ext, dst))
        return 0;
    if (bits == 16)
        return emit16(ctx, (unsigned int)imm);
...
static int infer_bits_from_operand(operand_t *op, int fallback) {
    if (op->kind == OP_REG)
        return op->reg_bits;
    if (op->kind == OP_MEM && op->mem.size_hint)
        return op->mem.size_hint;
    return fallback;
}

static int encode_instruction(asm_ctx_t *ctx, char *mn, operand_t *ops,
                              int opn) {
    int cc = 0;
    int imm = 0;

    if (streqi(mn, "nop") && opn == 0)
        return emit8(ctx, 0x90);
    if (streqi(mn, "ret") && opn == 0)
        return emit8(ctx, 0xC3);
    if (streqi(mn, "leave") && opn == 0)
        return emit8(ctx, 0xC9);
    if (streqi(mn, "cdq") && opn == 0)
        return emit8(ctx, 0x99);
    if (streqi(mn, "cbw") && opn == 0)
        return emit8(ctx, 0x98);
    if (streqi(mn, "pushad") && opn == 0)
        return emit8(ctx, 0x60);
    if (streqi(mn, "popad") && opn == 0)
        return emit8(ctx, 0x61);
    if (streqi(mn, "iret") && opn == 0)
        return emit8(ctx, 0xCF);

    if (streqi(mn, "int") && opn == 1) {
        if (!resolve_imm(ctx, &ops[0], &imm))
            return 0;
    ...
    if (streqi(mn, "push") && opn == 1) {
        if (ops[0].kind == OP_REG)
            return emit8(ctx, (unsigned int)(0x50 + ops[0].reg));
        if (ops[0].kind == OP_IMM || ops[0].kind == OP_LABEL) {
            if (ctx->fmt_obj && ops[0].kind == OP_LABEL) {
                ...
                unsigned int off = ctx->sec_pc[ctx->cur_sec];
                if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                               MOBJ_RELOC_ABS32, ops[0].label, 0))
                    return 0;
                return emit32(ctx, 0);
            }
            if (!resolve_imm(ctx, &ops[0], &imm))
                return 0;
            if (imm >= -128 && imm <= 127)
                return emit8(ctx, 0x6A) &&
                       emit8(ctx, (unsigned int)(imm & 0xFF));
            return emit8(ctx, 0x68) && emit32(ctx, (unsigned int)imm);
        }
        if (ops[0].kind == OP_MEM)
            return emit8(ctx, 0xFF) && emit_rm_operand(ctx, 6, &ops[0]);
    }

    if (streqi(mn, "pop") && opn == 1) {
        if (ops[0].kind == OP_REG)
            return emit8(ctx, (unsigned int)(0x58 + ops[0].reg));
        if (ops[0].kind == OP_MEM)
            return emit8(ctx, 0x8F) && emit_rm_operand(ctx, 0, &ops[0]);
    }
    ...
    if (streqi(mn, "call") && opn == 1) {
        if (ops[0].kind == OP_LABEL || ops[0].kind == OP_IMM) {
            if (ctx->fmt_obj && ops[0].kind == OP_LABEL) {
                ...
                unsigned int off = ctx->sec_pc[ctx->cur_sec];
                if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                               MOBJ_RELOC_REL32, ops[0].label, 0))
                    return 0;
                return emit32(ctx, 0);
            }
            if (!resolve_imm(ctx, &ops[0], &imm))
    ...
    if (is_jcc(mn, &cc) && opn == 1) {
        if (ctx->fmt_obj && ops[0].kind == OP_LABEL) {
            if (!emit8(ctx, 0x0F) || !emit8(ctx, (unsigned int)(0x80 + cc)))
                return 0;
            unsigned int off = ctx->sec_pc[ctx->cur_sec];
            if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                           MOBJ_RELOC_REL32, ops[0].label, 0))
        ...
        return emit8(ctx, 0x0F) && emit8(ctx, (unsigned int)(0x80 + cc)) &&
               emit32(ctx, (unsigned int)rel);
    }
    ...
    if (is_setcc(mn, &cc) && opn == 1) {
        ...
        return emit8(ctx, 0x0F) && emit8(ctx, (unsigned int)(0x90 + cc)) &&
               emit_rm_operand(ctx, 0, &ops[0]);
    }

    if (streqi(mn, "inc") && opn == 1)
        return emit8(ctx, 0xFF) && emit_rm_operand(ctx, 0, &ops[0]);
    if (streqi(mn, "dec") && opn == 1)
        return emit8(ctx, 0xFF) && emit_rm_operand(ctx, 1, &ops[0]);
    if (streqi(mn, "not") && opn == 1)
        return emit8(ctx, 0xF7) && emit_rm_operand(ctx, 2, &ops[0]);
    if (streqi(mn, "neg") && opn == 1)
        return emit8(ctx, 0xF7) && emit_rm_operand(ctx, 3, &ops[0]);
    if (streqi(mn, "mul") && opn == 1)
        return emit8(ctx, 0xF7) && emit_rm_operand(ctx, 4, &ops[0]);
    if (streqi(mn, "imul") && opn == 1)
        return emit8(ctx, 0xF7) && emit_rm_operand(ctx, 5, &ops[0]);
    if (streqi(mn, "idiv") && opn == 1)
        return emit8(ctx, 0xF7) && emit_rm_operand(ctx, 7, &ops[0]);

    if (streqi(mn, "mov") && opn == 2) {
        int bits = infer_bits_from_operand(
            &ops[0], infer_bits_from_operand(&ops[1], 32));
        if (ops[0].kind == OP_REG &&
            (ops[1].kind == OP_IMM || ops[1].kind == OP_LABEL)) {
            if (ctx->fmt_obj && ops[1].kind == OP_LABEL && bits == 32) {
                if (!maybe_prefix_16(ctx, bits))
                    return 0;
                if (!emit8(ctx, (unsigned int)(0xB8 + ops[0].reg)))
                    return 0;
                unsigned int off = ctx->sec_pc[ctx->cur_sec];
                if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                               MOBJ_RELOC_ABS32, ops[1].label, 0))
                    return 0;
                return emit32(ctx, 0);
            }
            if (!resolve_imm(ctx, &ops[1], &imm))
                return 0;
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            return emit8(ctx, (unsigned int)(0xB8 + ops[0].reg)) &&
                   (bits == 16 ? emit16(ctx, (unsigned int)imm)
                               : emit32(ctx, (unsigned int)imm));
        }
        if (ops[0].kind == OP_REG &&
            (ops[1].kind == OP_REG || ops[1].kind == OP_MEM)) {
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            return emit8(ctx, (bits == 8) ? 0x8A : 0x8B) &&
                   emit_rm_operand(ctx, ops[0].reg, &ops[1]);
        }
        if ((ops[0].kind == OP_REG || ops[0].kind == OP_MEM) &&
            ops[1].kind == OP_REG) {
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            return emit8(ctx, (bits == 8) ? 0x88 : 0x89) &&
                   emit_rm_operand(ctx, ops[1].reg, &ops[0]);
        }
        if (ops[0].kind == OP_MEM &&
            (ops[1].kind == OP_IMM || ops[1].kind == OP_LABEL)) {
            if (ctx->fmt_obj && ops[1].kind == OP_LABEL) {
                ...
                if (!maybe_prefix_16(ctx, bits))
                    return 0;
                if (!emit8(ctx, 0xC7) || !emit_rm_operand(ctx, 0, &ops[0]))
                    return 0;
                unsigned int off = ctx->sec_pc[ctx->cur_sec];
                if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                               MOBJ_RELOC_ABS32, ops[1].label, 0))
            ...
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            if (bits == 8)
                return emit8(ctx, 0xC6) && emit_rm_operand(ctx, 0, &ops[0]) &&
                       emit8(ctx, (unsigned int)(imm & 0xFF));
    ...
    if (streqi(mn, "lea") && opn == 2 && ops[0].kind == OP_REG &&
        ops[1].kind == OP_MEM) {
    ...
    if ((streqi(mn, "movsx") || streqi(mn, "movzx")) && opn == 2 &&
        ops[0].kind == OP_REG &&
        (ops[1].kind == OP_REG || ops[1].kind == OP_MEM)) {
        int src_bits = infer_bits_from_operand(
            &ops[1], ops[1].kind == OP_REG ? ops[1].reg_bits : 8);
        int op2 = streqi(mn, "movsx") ? 0xBE : 0xB6;
        if (src_bits == 16)
            op2++;
        return emit8(ctx, 0x0F) && emit8(ctx, (unsigned int)op2) &&
               emit_rm_operand(ctx, ops[0].reg, &ops[1]);
    }

    if (streqi(mn, "imul") && opn == 2 && ops[0].kind == OP_REG &&
        (ops[1].kind == OP_REG || ops[1].kind == OP_MEM)) {
        return emit8(ctx, 0x0F) && emit8(ctx, 0xAF) &&
               emit_rm_operand(ctx, ops[0].reg, &ops[1]);
    }
    ...
    if (streqi(mn, "imul") && opn == 3 && ops[0].kind == OP_REG &&
        ...
        if (!resolve_imm(ctx, &ops[2], &imm))
            return 0;
        if (imm >= -128 && imm <= 127) {
            return emit8(ctx, 0x6B) &&
                   emit_rm_operand(ctx, ops[0].reg, &ops[1]) &&
                   emit8(ctx, (unsigned int)(imm & 0xFF));
        }
        return emit8(ctx, 0x69) && emit_rm_operand(ctx, ops[0].reg, &ops[1]) &&
               emit32(ctx, (unsigned int)imm);
    }
    ...
    struct {
        ...
        int grp_ext;
    } alu_ops[] = {{"add", 0x01, 0x03, 0}, {"or", 0x09, 0x0B, 1},
    ...
    for (unsigned int i = 0; i < sizeof(alu_ops) / sizeof(alu_ops[0]); i++) {
        ...
        if ((ops[0].kind == OP_REG || ops[0].kind == OP_MEM) &&
            ...
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            return encode_binop_rm_reg(
                ctx,
                (unsigned char)(bits == 8 ? (alu_ops[i].rm_reg - 1)
                                          : alu_ops[i].rm_reg),
                &ops[0], &ops[1]);
        }
        if (ops[0].kind == OP_REG &&
            ...
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            return encode_binop_reg_rm(
                ctx,
                (unsigned char)(bits == 8 ? (alu_ops[i].reg_rm - 1)
                                          : alu_ops[i].reg_rm),
                &ops[0], &ops[1]);
        }
        if ((ops[0].kind == OP_REG || ops[0].kind == OP_MEM) &&
    ...
    if (streqi(mn, "test") && opn == 2) {
        ...
        if ((ops[0].kind == OP_REG || ops[0].kind == OP_MEM) &&
            ...
            if (!resolve_imm(ctx, &ops[1], &imm))
                return 0;
            if (bits == 8)
                return emit8(ctx, 0xF6) && emit_rm_operand(ctx, 0, &ops[0]) &&
                       emit8(ctx, (unsigned int)(imm & 0xFF));
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            if (!emit8(ctx, 0xF7))
                return 0;
            if (!emit_rm_operand(ctx, 0, &ops[0]))
                return 0;
            if (bits == 16)
                return emit16(ctx, (unsigned int)imm);
            return emit32(ctx, (unsigned int)imm);
    ...
    if ((streqi(mn, "shl") || streqi(mn, "sal") || streqi(mn, "shr") ||
         streqi(mn, "sar") || streqi(mn, "rol") || streqi(mn, "ror")) &&
        opn == 2) {
        int ext = 0;
        if (streqi(mn, "rol"))
            ext = 0;
        else if (streqi(mn, "ror"))
            ext = 1;
        else if (streqi(mn, "shl") || streqi(mn, "sal"))
            ext = 4;
        else if (streqi(mn, "shr"))
            ext = 5;
        else if (streqi(mn, "sar"))
            ext = 7;
        int bits = infer_bits_from_operand(&ops[0], 32);
        if (ops[1].kind == OP_REG && ops[1].reg_bits == 8 && ops[1].reg == 1) {
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            if (!emit8(ctx, bits == 8 ? 0xD2 : 0xD3))
                return 0;
        ...
        if (ops[1].kind == OP_IMM || ops[1].kind == OP_LABEL) {
            ...
            if (!maybe_prefix_16(ctx, bits))
                return 0;
            if (!emit8(ctx, bits == 8 ? 0xC0 : 0xC1))
                return 0;
            if (!emit_rm_operand(ctx, ext, &ops[0]))
                return 0;
            return emit8(ctx, (unsigned int)(imm & 0xFF));
        }
    }
...
static int emit_data_item(asm_ctx_t *ctx, const char *tok, int bytes) {
    ...
    if (!ok) {
        if (ctx->fmt_obj && bytes == 4 && ctx->pass == 2) {
            unsigned int off = ctx->sec_pc[ctx->cur_sec];
            if (!add_reloc(ctx, (unsigned int)ctx->cur_sec, off,
                           MOBJ_RELOC_ABS32, s, 0))
                return 0;
            val = 0;
            ok = 1;
        }
        // label value
        if (!ok && !resolve_label(ctx, s, &val) && ctx->pass == 1)
            val = 0;
    }
    if (bytes == 1)
        return emit8(ctx, (unsigned int)(val & 0xFF));
    if (bytes == 2)
        return emit16(ctx, (unsigned int)(val & 0xFFFF));
...
static int process_line(asm_ctx_t *ctx, char *line) {
    ...
    if (streqi(mnem, "global") || streqi(mnem, "extern")) {
        ...
        for (int i = 0; i < n; i++) {
            if (!toks[i][0])
                continue;
            if (streqi(mnem, "global"))
                mark_label_global(ctx, toks[i]);
            else
                mark_label_extern(ctx, toks[i]);
        }
        return 1;
    }
    ...
    if (streqi(mnem, "section")) {
        ...
        else if (streqi(p, "rodata"))
            ctx->cur_sec = SEC_RODATA;
        else if (streqi(p, "data"))
            ctx->cur_sec = SEC_DATA;
        else if (streqi(p, "bss"))
            ctx->cur_sec = SEC_BSS;
        else {
    ...
    if (streqi(mnem, "org")) {
        ...
        if (ctx->sec_pc[SEC_TEXT] == 0 && ctx->sec_pc[SEC_RODATA] == 0 &&
            ctx->sec_pc[SEC_DATA] == 0 && ctx->sec_pc[SEC_BSS] == 0) {
    ...
    if (streqi(mnem, "times")) {
        ...
        int bytes = 0;
        if (streqi(parts[1], "db"))
            bytes = 1;
        else if (streqi(parts[1], "dw"))
            bytes = 2;
        else if (streqi(parts[1], "dd"))
            bytes = 4;
        else {
        ...
        for (int i = 0; i < count; i++) {
            if (!emit_data_item(ctx, tail, bytes))
                return 0;
        }
        return 1;
    }
    ...
    if (streqi(mnem, "db") || streqi(mnem, "dw") || streqi(mnem, "dd")) {
        ...
        int n = split_operands(rest, toks, 64);
        for (int i = 0; i < n; i++) {
            if (!emit_data_item(ctx, toks[i], bytes))
                return 0;
        }
        return 1;
    }
...
static int assemble(asm_ctx_t *ctx) {
    ...
    ctx->sec_base[SEC_RODATA] =
        ctx->sec_base[SEC_TEXT] + ctx->sec_size[SEC_TEXT];
    ctx->sec_base[SEC_DATA] =
        ctx->sec_base[SEC_RODATA] + ctx->sec_size[SEC_RODATA];
    ...
    if (need == 0 && ctx->meaningful_lines > 0) {
        print_err(
            "no encodable output (0 bytes); likely unsupported asm forms");
        ctx->had_error = 1;
        return 0;
    }
    ...
    for (int s = 0; s < SEC_COUNT; s++) {
        if (ctx->sec_len[s] > 0) {
            memcpy(ctx->out + ctx->out_len, ctx->sec_out[s],
                   (unsigned int)ctx->sec_len[s]);
...
static void usage(void) {
    print("usage: as86 [-f bin|obj] [--org addr] [-o out] <input.asm> "
          "[output]\n");
...
void _start(int argc, char **argv) {
    ...
    if (!fmt_obj) {
        if (fd_write(ofd, ctx->out, (unsigned int)ctx->out_len) !=
            ctx->out_len) {
            close(ofd);
        ...
        h.entry_off = 0;
        unsigned int ent = 0;
        if (find_label_addr(ctx, "$_start", &ent) ||
            find_label_addr(ctx, "_start", &ent)) {
        ...
        if (ctx->sec_len[SEC_TEXT] &&
            fd_write(ofd, ctx->sec_out[SEC_TEXT],
                     (unsigned int)ctx->sec_len[SEC_TEXT]) !=
                ctx->sec_len[SEC_TEXT]) {
            close(ofd);
            print_err("write failed");
            exit(1);
        }
        if (ctx->sec_len[SEC_RODATA] &&
            fd_write(ofd, ctx->sec_out[SEC_RODATA],
                     (unsigned int)ctx->sec_len[SEC_RODATA]) !=
                ctx->sec_len[SEC_RODATA]) {
            close(ofd);
            print_err("write failed");
            exit(1);
        }
        if (ctx->sec_len[SEC_DATA] &&
            fd_write(ofd, ctx->sec_out[SEC_DATA],
                     (unsigned int)ctx->sec_len[SEC_DATA]) !=
                ctx->sec_len[SEC_DATA]) {
            close(ofd);
            print_err("write failed");
            exit(1);
        }
        for (int i = 0; i < ctx->label_count; i++) {
            ...
            if (ctx->labels[i].is_global)
                s.flags |= MOBJ_SYM_GLOBAL;
            if (ctx->labels[i].is_extern || !ctx->labels[i].defined)
                s.flags |= MOBJ_SYM_EXTERN;
        ...
        for (int i = 0; i < ctx->reloc_count; i++) {
            if (fd_write(ofd, &ctx->relocs[i], sizeof(ctx->relocs[i])) !=
                (int)sizeof(ctx->relocs[i])) {
                close(ofd);
                print_err("write failed");
                exit(1);
            }
    ...
    if (!fmt_obj) {
        ...
        int total = (int)sizeof(mobj_header_t) + ctx->sec_len[SEC_TEXT] +
                    ctx->sec_len[SEC_RODATA] + ctx->sec_len[SEC_DATA] +
                    (int)sizeof(mobj_sym_t) * ctx->label_count +
                    (int)sizeof(mobj_reloc_t) * ctx->reloc_count;
        print_num(total);
    }
    print(" bytes to ");
    print(out);
