...
// 16-bit desktop path still uses indexed colors in window buffers; choose
// richer palette entries.
#define COL_BG 237
...
static void sort_ext_filters(void) {
    for (int i = 0; i < ext_filter_count; i++) {
        ...
        for (int j = i + 1; j < ext_filter_count; j++) {
            if (strcmp(ext_filters[j], ext_filters[best]) < 0)
                best = j;
        }
        if (best != i) {
            char tmp[EXT_MAX];
...
static void rebuild_ext_filters(void) {
    ...
    sort_ext_filters();
    if (ext_filter_idx > ext_filter_count)
        ext_filter_idx = 0;
}
...
static void buf_vline(int x, int y, int h, unsigned char c) {
    for (int i = 0; i < h; i++)
        ugfx_buf_pixel(buf, W, H, x, y + i, c);
}

static void draw_bitmap16(int x, int y, const unsigned short *rows,
                          unsigned char fg) {
...
static void draw_file_icon(int x, int y, int selected_cell, const char *name,
    ...
    ugfx_buf_rect(buf, W, H, ix + 1, iy + 1, 22, 16,
                  selected_cell ? COL_SEL_BG : COL_PANEL_ALT);
...
static void ensure_selected_visible(void) {
    ...
    if (file_count <= page) {
        ...
        if (view_first > max_first)
            view_first = max_first;
    }
}
...
static void redraw(void) {
    ...
    info[p++] = ']';
    info[p] = '\0';
    ugfx_buf_string(buf, W, H, W - (p * 8) - 4, H - STATUS_H + 3, info,
                    COL_MUTED);
}
...
static void cycle_filter(void) {
    ...
    if (ext_filter_idx == 0) {
        ...
        while (ext[j] && p < (int)sizeof(msg) - 1)
            msg[p++] = ext[j++];
        msg[p] = '\0';
    }
    copy_status(msg);
...
void _start(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (k == 'r' || k == 'R') {
            ...
            int cols, rows, page;
            grid_dims(&cols, &rows, &page);
            if (k == 'a' || k == 'A' || k == KEY_LEFT)
                move_selection(-1);
            if (k == 'd' || k == 'D' || k == KEY_RIGHT)
                move_selection(1);
            if (k == 'w' || k == 'W' || k == KEY_UP)
                move_selection(-cols);
