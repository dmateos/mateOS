...
wad_file_t *W_AddFile (char *filename)
{
    ...
    else 
    {
        ...
        // Defensive checks for malformed/failed header reads on bring-up.
        if (header.numlumps <= 0 || header.numlumps > 20000)
        {
            I_Error("WAD header numlumps invalid: %d", header.numlumps);
        }
        if (header.infotableofs < 0)
        {
            I_Error("WAD header infotableofs invalid: %d", header.infotableofs);
        }
    ...
    startlump = numlumps;
    if (newnumlumps <= numlumps || newnumlumps > 20000)
    {
        I_Error("newnumlumps invalid: old=%d new=%d", (int)numlumps, newnumlumps);
    }
