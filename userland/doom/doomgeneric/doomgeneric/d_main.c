...
void D_DoomMain (void)
{
    ...
    // mateOS: force local config dir and avoid platform-specific probing.
    M_SetConfigDir(".");
    ...
    // mateOS bring-up path: skip config file IO and use direct IWAD lookup.
    M_SetConfigDir(".");
    M_SetConfigFilenames("default.cfg", PROGRAM_PREFIX "doom.cfg");
    D_BindVariables();

    // Find main IWAD file directly to avoid platform path/env complexity.
    // Bypass D_FindWADByName on mateOS for now (it stalls in current port).
    iwadfile = "DOOM1.WAD";
    gamemission = doom;
...
//    W_PrintDirectory();
    ...
    I_InitTimer();
    I_InitJoystick();
    // mateOS: disable low-level sound/music backend init for now.
    // Even with -nosound flags, backend init paths can still stall here.
    ...
    // Keep game-side sound tables init disabled until backend is stable.
    // This allows video/game loop bring-up first.
